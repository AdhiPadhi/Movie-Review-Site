...
} //if the user is not logged in, the new review card will be hidden, since only logged-in users should be able to add new reviews, this is done by setting the display style of the new review card to "none" if there is no logged-in user, this will ensure that only authenticated users can see the form to add a new review and submit it to the backend API
...
<div class="row">
    <div class="column">
        <div class="card">
            ...
            <p><a href="#" onclick="saveReview('new_review')">💾</a></p>
        </div>
    </div>
  ...
  credentials: "include"
    ...
    .then(function(data){
        ...
        data.forEach(review => {
            ...
            if (loggedUser && review.user === loggedUser.email) { //checking if the user is logged in and if the review belongs to the logged-in user, this is done to determine whether to show the edit and delete buttons for the review, only the original poster of the review should be able to see the edit and delete buttons for their review
            div_card.innerHTML = `
            <div class="row">
                <div class="column">
            ...
            else{ //if the user is not logged in or if the review does not belong to the logged-in user, the edit and delete buttons will not be shown for that review
                div_card.innerHTML = `
            <div class="row">
                <div class="column">
                    <div class="card" id=${review._id}>
            ...
            `;} //setting the inner HTML of the review card to display the review text and user name, without the edit and delete buttons, since the user is not logged in or does not own the review
...
function saveReview(reviewInputId, id="") { //defining a function to handle the save review action, this function will be called when the submit button for editing a review is clicked, it takes the IDs of the review input field, user input field, and review ID as parameters and saves the updated review to the database
    ...
    if (id) { //if the review ID is provided, it means that we are updating an existing review, so a PUT request will be made to the backend API to update the review in the database
        fetch(APILINK + id, { //making a PUT request to the backend API to update the review in the database, the URL is constructed by appending the review ID to the base API URL, this will make a PUT request to the /api/v1/reviews/:id endpoint defined in the reviews.route.js file, where :id is replaced with the actual review ID)
            ...
            body: JSON.stringify({ "review": review }) //the request body contains the updated review text and user name in JSON format, this will be sent to the backend API to update the review in the database
        }).then(res => res.json())
        .then(res => {
            console.log(res);
        ...
        fetch(APILINK + "new", { //making a POST request to the backend API to add the new review to the database, this will make a POST request to the /api/v1/reviews/new endpoint defined in the reviews.route.js file
            ...
            body: JSON.stringify({"review": review, "movieId": movieId }) //the request body contains the movie ID, review text, and user name in JSON format, this will be sent to the backend API to add the new review to the database
        }).then(res => res.json())
        .then(res => {
            console.log(res);
